...
{
    const double phi =
        0.7 * std::cos(x) + 0.5 * std::cos(2.0 * y) + 0.3 * std::cos(3.0 * z) + 0.2 * std::sin(x + y) + 0.1 * std::sin(y + z) + 0.05 * std::cos(28.0 * x) + 0.05 * std::sin(27.0 * (y + z));
...
__host__ __device__ inline double rhsFunction(double x, double y, double z)
{
    ...
    const double phix =
        -0.7 * std::sin(x) + 0.2 * std::cos(x + y) - 0.05 * 28.0 * std::sin(28.0 * x);

    const double phiy =
        -1.0 * std::sin(2.0 * y) + 0.2 * std::cos(x + y) + 0.1 * std::cos(y + z) + 0.05 * 27.0 * std::cos(27.0 * (y + z));

    const double phiz =
        -0.9 * std::sin(3.0 * z) + 0.1 * std::cos(y + z) + 0.05 * 27.0 * std::cos(27.0 * (y + z));

    const double phixx =
        -0.7 * std::cos(x) - 0.2 * std::sin(x + y) - 0.05 * 28.0 * 28.0 * std::cos(28.0 * x);

    const double phiyy =
        -2.0 * std::cos(2.0 * y) - 0.2 * std::sin(x + y) - 0.1 * std::sin(y + z) - 0.05 * 27.0 * 27.0 * std::sin(27.0 * (y + z));

    const double phizz =
        -2.7 * std::cos(3.0 * z) - 0.1 * std::sin(y + z) - 0.05 * 27.0 * 27.0 * std::sin(27.0 * (y + z));
 ...
 */
static __global__ void initRhsKernel(hipfftDoubleComplex *d_data,
 ...
 */
static __global__ void poissonFourierKernel(hipfftDoubleComplex *d_data,
                                            size_t Nx, size_t Ny, size_t Nz)
{
    size_t i = blockIdx.x * blockDim.x + threadIdx.x;
    size_t j = blockIdx.y * blockDim.y + threadIdx.y;
 ...
 */
static __global__ void normalizeKernel(hipfftDoubleComplex *d_data,
...
int main(int argc, char *argv[])
{
    ...
    double gpu_l2 = std::transform_reduce(std::execution::par, gpu_errors.begin(), gpu_errors.end(), 0.0, std::plus<>(), [](double e)
                                          { return e * e; });
    double cpu_l2 = std::transform_reduce(std::execution::par, cpu_errors.begin(), cpu_errors.end(), 0.0, std::plus<>(), [](double e)
                                          { return e * e; });
